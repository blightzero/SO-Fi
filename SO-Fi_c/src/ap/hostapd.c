...
static int hostapd_validate_bssid_configuration(struct hostapd_iface *iface)
{
	...
	/* Make sure that additional BSSIDs fit for pwifi. */
	if (i < PWIFI_MAX_BSS_COUNT) {
	    i = PWIFI_MAX_BSS_COUNT;
	}
...
int hostapd_setup_bss(struct hostapd_data *hapd, int first)
{
	...
	u8 if_addr[ETH_ALEN];
	struct os_time hostapd_setup_psk_wpa_timer;
	...
	pwifi_start_timer(&hostapd_setup_psk_wpa_timer, NULL);
	if (hostapd_setup_wpa_psk(conf)) {
		wpa_printf(MSG_ERROR, "WPA-PSK setup failed.");
		return -1;
	}
	pwifi_elapsed_time(&hostapd_setup_psk_wpa_timer, "hostapd_setup_psk_wpa:");
...
void hostapd_interface_deinit(struct hostapd_iface *iface)
{
	...
	/* This is a workaround to avoid a crash caused by an upstream bug where
	 * hostapd segfaults when more than one BSS has a station connected on exit.
	 * Remove all stations from the BSS and call our pwifi station timeout routine
	 * which then removes the BSS as no station is left connected. */
	for (j = iface->num_bss - 1; j > 0; j--) {
	    struct hostapd_data *hapd = iface->bss[j];
	    hostapd_free_stas(hapd);
	    pwifi_handle_sta_timeout(hapd, NULL);
	}
